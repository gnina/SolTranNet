...
def make_model(d_atom=28, N=8, d_model=8, h=2, dropout=0.1, lambda_attention=0.5,
    ...
    if torch.cuda.is_available():
        ...
        model.load_state_dict(torch.load('weights/soltrannet_aqsol_trained.weights',map_location=device))
    else:
        device=torch.device("cuda")
        model.load_state_dict(torch.load('weights/soltrannet_aqsol_trained.weights'))
        model.to(device)
