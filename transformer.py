...
def make_model(d_atom, N=2, d_model=128, h=8, dropout=0.1, 
               lambda_attention=0.3, trainable_lambda=False,
               N_dense=2, leaky_relu_slope=0.0, aggregation_type='mean', 
               dense_output_nonlinearity='relu',
               use_edge_features=False, n_output=1, control_edges=False,
               scale_norm=False, init_type='uniform', use_adapter=False, n_generator_layers=1):
    "Helper: Construct a model from hyperparameters."
    c = copy.deepcopy
    attn = MultiHeadedAttention(h, d_model, dropout, lambda_attention, trainable_lambda, use_edge_features, control_edges)
...
def attention(query, key, value, adj_matrix, edges_att,
              ...
              lambdas=(0.5, 0.5), trainable_lambda=False,
              use_edge_features=False, control_edges=False,
              eps=1e-6, inf=1e12):
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, lambda_attention=0.5, trainable_lambda=False, 
                 use_edge_features=False, control_edges=False):
        ...
        if trainable_lambda:
            ...
            lambdas_tensor = torch.tensor([lambda_attention, lambda_adjacency], requires_grad=True)
